...
1. **Update design tokens** (if needed):
   - Export from Figma and replace `src/tokens/tokens.json` (do not edit this file manually)
...
2. **Make your changes** to components:
   ...
   - Typography/shape/elevation: prefer MUI theme tokens (and `src/utils/typography.ts`, `src/utils/shapes.ts`) over hardcoded values
